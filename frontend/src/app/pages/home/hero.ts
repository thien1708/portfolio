...
@Component({
  ...
  template: `
    <section id="hero" class="relative flex min-h-screen items-center overflow-hidden pt-24">
      ...
      <div class="pointer-events-none absolute inset-0 -z-10">
        ...
        <!-- Soft mesh gradient drifts up on scroll (native scroll parallax) -->
        <div class="parallax-slow hero-mesh absolute inset-0"></div>
      ...
      <div class="mx-auto grid w-full max-w-6xl items-center gap-12 px-6 pb-16 lg:grid-cols-[1.2fr_1fr]">
        <div class="order-2 text-center lg:order-1 lg:text-left">
          @if (profile(); as p) {
            ...
            <h1 appSpotlight class="hero-title -mx-3 rounded-2xl px-3 font-display font-extrabold leading-tight tracking-tight">
            ...
            <div appReveal [revealDelay]="200" class="mt-4 flex h-9 items-center justify-center font-display text-xl font-semibold text-lav-700 dark:text-lav-200 sm:text-2xl lg:justify-start">
              <span>{{ typed() }}</span>
              <span class="ml-0.5 inline-block h-6 w-[3px] rounded bg-coral-400 animate-blink sm:h-7"></span>
            </div>
            <p appReveal [revealDelay]="300" class="mx-auto mt-6 max-w-xl text-base leading-relaxed text-ink/80 dark:text-lav-100/80 lg:mx-0">
              {{ p.summary }}
            </p>
            ...
            <div appReveal="scale" [appTilt]="10" class="relative">
              ...
              @if (profile()?.avatarUrl) {
                <img
                  ...
                  fetchpriority="high"
                  class="hero-avatar relative h-56 w-56 rounded-full object-cover sm:h-72 sm:w-72"
                />
              } @else {
                <div class="hero-avatar relative grid h-56 w-56 place-items-center rounded-full bg-gradient-to-br from-lav-100 to-sky2-100 font-display text-6xl font-extrabold text-lav-600 dark:from-lav-800 dark:to-[#16213a] dark:text-lav-200 sm:h-72 sm:w-72">
                  {{ initials() }}
                </div>
              }
            </div>
  ...
  `,
  imports: [RevealDirective, Icon, MagneticDirective, TiltDirective, SplitTextDirective, SpotlightDirective, HeroScene],
})
