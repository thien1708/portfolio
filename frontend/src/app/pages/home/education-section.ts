      ...
      <div class="grid gap-8 lg:grid-cols-2">
        <div appReveal="left" class="space-y-6">
          <h3 class="flex items-center gap-3 font-display text-xl font-bold">
            <span class="glass grid h-11 w-11 place-items-center rounded-2xl text-xl">🎓</span>
            Education
          </h3>
          @if (education().length > 0) {
            @for (item of education(); track item.id) {
              <div class="card group p-6 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-glow">
                @if (item.period) {
                  <span class="chip mb-3">🗓 {{ item.period }}</span>
                }
        ...
        <div appReveal="right" class="space-y-6">
          ...
          @if (certifications().length > 0) {
            @for (cert of certifications(); track cert.id) {
              <div class="card group flex items-center gap-4 p-6 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-glow">
                <span class="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-lav-200 to-sky2-200 text-2xl transition-transform duration-300 group-hover:scale-110 group-hover:rotate-6 dark:from-lav-800 dark:to-sky2-500/30">🏆</span>
                <div>
                  ...
                  @if (cert.url) {
                    <a [href]="cert.url" target="_blank" rel="noopener"
                       class="mt-1 inline-block text-xs font-semibold text-lav-600 hover:underline dark:text-lav-300">
                      View credential →
                    </a>
                  }
                </div>
